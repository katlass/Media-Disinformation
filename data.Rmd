...
dataset = data
dataset$participant_id=1:nrow(dataset)
print(as.data.frame(select(dataset[!complete.cases(dataset),],c(participant_id,na_cols))))
```

Now we look to investigate whether there were any similarities among these four participants:
```{r}
missing_obs=select(dataset[!complete.cases(dataset),],c(participant_id,na_cols,'gender', 'pol_party', 'ideology', 'condition','age',  'AOT'))
print(missing_obs)
```
...
```{r}
library(redav)
plot_missing(na_data, percent = FALSE)
```
