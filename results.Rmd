...
#### Age vs Total Fact Checks by Ideology and Gender - STILL UNSURE ABOUT THIS ONE
```{r}
class(dataset$age)
...
#   ggplot(aes(x = ideology, y = total_fact_checks)) +
...
```{r}
library(ggridges)
# Split participants into two artificial categories representing their ideologies, 
# according to the "ideology" variable. This split into only two categories will be 
# useful for the group of charts below:


# dataset$ideology <- as.numeric(dataset$ideology)
# dataset$id_group = ifelse(dataset$ideology <= 3, "Liberal", "Conservative")
p1 <- ggplot(dataset, aes(x = AOT, y = ideology, fill = ideology, color = ideology)) +
  geom_density_ridges() +
  scale_fill_manual(values = c("#277DA1",
                               "#8C9ACC",
                               "#95D1FF",
